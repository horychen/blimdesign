...
def read_forceXY_data(XY='X'):
    ...
    ax.grid(True)
    ax.set_ylabel('%s-axis force [N]'%(XY), fontsize=17)
    ax.set_xlabel('Slip frequency [Hz]', fontsize=17)
...
if True:
    ...
    fig, ax_list = subplots(1, 2, figsize=(6,3), dpi=150)
...
def get_angle_plot(line1, line2, offset = 1, color = None, origin = [0,0], len_x_axis = 1, len_y_axis = 1):
    ...
    return Arc(origin, len_x_axis*offset, len_y_axis*offset, 0, theta1, theta2, color=color, label = str(angle)+u"\u00b0", zorder=99)
def get_angle_text(angle_plot, bool_show_value=False):
    ...
    x_width = abs(vertices[0][0] + vertices[-1][0]) / 2.0 + 0.080
    y_width = abs(vertices[0][1] + vertices[-1][1]) / 2.0 - 0.03
...
if True:
    ...
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_ylabel('Y-axis force', fontsize=17)
    ax.set_xlabel('X-axis force', fontsize=17)
    ax.set_xlim([0, 0.5])
    ax.set_ylim([0, 0.6])
...
ax.annotate(
    ...
    xy=(avg[0]-0.035, avg[1]-0.04), arrowprops=dict(arrowstyle='->',color='r'), xytext=(avg[0]-0.1, avg[1]-0.2), color='r', fontsize=17)

ax.annotate(
    'The most deviated\nforce vector',
    xy=(0.43018469, 0.17636313), arrowprops=dict(arrowstyle='->',color='k'), xytext=(0.43018469-0.08, 0.17636313-0.12), color='k', fontsize=17)

angle_plot = get_angle_plot(avg_line, lines[-1], 0.3, color='b')
angle_text = get_angle_text(angle_plot) 
ax.add_patch(angle_plot) # To display the angle arc
print angle_text
ax.text(*angle_text, color='b', rotation=20, fontsize=17)
