...
def write_Torque_and_B_data_to_file(str_rotor_position, rotation_operator):
    ...
    # option 1
    handle_stator_B_data.write(str_rotor_position + ',' + ','.join(['%g,%g,%g'%(Bx,By,A) for Bx,By,A in izip(stator_Bx_data, stator_By_data, stator_Area_data) ]) + '\n')
    handle_rotor_B_data.write(str_rotor_position  + ',' + ','.join(['%g,%g,%g'%(Bx,By,A) for Bx,By,A in izip(rotor_Bx_data, rotor_By_data, rotor_Area_data) ]) + '\n')
