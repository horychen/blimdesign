...
mpl.rcParams['font.size'] = 11*0+15 # 26.0
...
def plot_data(fname_force, fname_torque, fig2, ax2, fig2_title, fig, ax_list, number_of_steps_2ndTSS, label='', marker='-ko'):
    ...
    if 'Over' in label:
        ...
        ax4eric.grid()
        fig4eric.savefig(r'C:\Users\horyc\Desktop/OverCurrentFRW.eps', format='eps', dpi=300)


    ax_list[0][0].legend(loc=(0.45,0.7))
    ax_list[0][0].set_xlabel('(a)')
    ax_list[0][1].set_xlabel('(b)')
    # ax_list[0][0].set_xlabel('Suspension Current [p.u.]\n(a)')
    # ax_list[0][1].set_xlabel('Suspension Current [p.u.]\n(b)')
